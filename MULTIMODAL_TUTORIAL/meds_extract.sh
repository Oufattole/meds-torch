...
function display_help() {
    ...
    echo "  MIMICIV_ECG_DIR                                Directory containing raw MIMIC-IV ecg files."
...
if [[ "$1" == "do_unzip=true" || "$1" == "do_unzip=false" || "$2" == "do_unzip=true" || "$2" == "do_unzip=false" || "$3" == "do_unzip=true" || "$3" == "do_unzip=false" || "$4" == "do_unzip=true" || "$4" == "do_unzip=false" ]]; then
...
export MIMICIV_ECG_DIR=$2
export MIMICIV_PRE_MEDS_DIR=$3
export MIMICIV_MEDS_COHORT_DIR=$4
shift 4
...
EVENT_CONVERSION_CONFIG_FP="$(pwd)/configs/ecg_event_configs.yaml"
PIPELINE_CONFIG_FP="$(pwd)/configs/extract_MIMIC.yaml"
PRE_MEDS_PY_FP="$(pwd)/pre_MEDS.py"
PRE_MEDS_ECG_PY_FP="$(pwd)/pre_meds_ecg.py"
...
# echo "Running pre-MEDS conversion."
# python "$PRE_MEDS_ECG_PY_FP" input_dir="$MIMICIV_ECG_DIR" cohort_dir="$MIMICIV_PRE_MEDS_DIR"
