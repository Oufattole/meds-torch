#!/usr/bin/env python
"""Functions for tokenizing MEDS dataset, specifically thetext_value column using a pretrained tokenizer."""
...
def extract_statics_and_schema(df: pl.LazyFrame) -> pl.LazyFrame:
    ...
    static = static.with_columns(
        pl.col("text_value")
        .map_elements(lambda x: tokenize_text(x), return_dtype=pl.List(pl.Int64), skip_nulls=False)
        .alias("text_value")
    )
...
def extract_seq_of_subject_events(df: pl.LazyFrame) -> pl.LazyFrame:
    ...
    return (
        ...
        .agg(pl.col("code").name.keep(), fill_to_nans("numeric_value").name.keep(), "text_value")
