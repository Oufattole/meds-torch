...
class BimodalPytorchDataset(SeedableMixin, torch.utils.data.Dataset, TimeableMixin):
    ...
    def __init__(self, cfg: DictConfig, split: str):
        ...
        self.read_notes('/ssd-shared/physionet.org/files/mimic-iv-note/2.2/note/discharge.csv.gz')
   
    def read_notes(self, path):
        notes = pd.read_csv(path)
        self.notes = notes[notes["subject_id"].isin([idx[0] for idx in self.index])]
    ...
    @TimeableMixin.TimeAs
    def load_subject_dynamic_databy_subject_id(self, subject_id: int):
        """Loads and returns the dynamic data slice for a given subject index, with subject ID and time
        range."""
        subject_id, st, end = [idx for idx in self.index if idx[0] == subject_id][0]
        shard = self.subj_map[subject_id]
        subject_idx = self.subj_indices[subject_id]
        subject_dynamic_data = JointNestedRaggedTensorDict.load_slice(
            Path(self.config.data_dir) / "data" / f"{shard}.nrt", subject_idx
    ...
    def _seeded_getitem(self, idx: int) -> dict[str, list[float]]:
        ...
        notes = self.notes.iloc[idx]

        subject_dynamic_data, subject_id, st, end = self.load_subject_dynamic_databy_subject_id(notes["subject_id"])
    ...
    def collate_triplet(cls, batch: list[dict], do_prepend_static_data=True) -> dict:
        ...
        tensorized_batch["notes"] = tokenized_text

        # Add task labels to batch
        for k in batch[0].keys():
            if k not in ("dynamic", "static_values", "static_indices", "notes"):
