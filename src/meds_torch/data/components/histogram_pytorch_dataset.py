...
from meds_torch.data.components.pytorch_dataset import PytorchDataset, DummyConfig
...
def insert_h_o_tokens_with_time_bins(codes: np.ndarray, time_deltas: np.ndarray, time_bin_size: float, h_token: int, o_token: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        tuple[np.ndarray, np.ndarray]: The resulting array with H and O tokens inserted and the number of empty bins.
...
def insert_h_o_tokens(codes, token_bin_size, h_token, o_token):
...
def get_segment_indices(inserted_codes: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
def convert_to_counts_batched(logits_batch: torch.Tensor, N: torch.Tensor) -> torch.Tensor:
    """Convert logits to counts by flooring the probabilities and then distributing the remaining counts to the top decimal parts.

    Args:
        logits_batch (torch.Tensor): batch of logits of shape [B, L]
        N (torch.Tensor): number of counts to use to convert logits to a histogram, accepts shape [B] or [B, 1]
...
class HistogramPytorchDataset(PytorchDataset):
    ...
    First let's look at what a batch looks like from this dataset
        ...
        >>> print(batch.keys())
        dict_keys(['code', 'mask', 'histogram', 'start_idx', 'end_idx', 'start_time', 'end_time', 'subject_id'])
    ...
    def _seeded_getitem(self, idx: int) -> dict:
        ...
        if self.cfg.token_insertion_strategy == TokenInsertionStrategy.TOKEN_COUNT:
            ...
            raise ValueError(f"Invalid token insertion strategy: {self.cfg.token_insertion_strategy}, should be one of {TokenInsertionStrategy}")
