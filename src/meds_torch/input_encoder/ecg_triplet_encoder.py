...
class ECGTripletEncoder(nn.Module, Module):
    ...
    Inputs:
        batch (dict): A dictionary containing the following keys:
            ...
            - "numeric_value_mask": Boolean tensor marking valid numeric values
            ...
            - "modality_sequence_idx": Tensor of positions in time series each ECG corresponds to
                (shape: [num_ecgs]).
    ...
    Example 1 -- early_fusion==True:
        ...
        >>> cfg = OmegaConf.create({"token_dim": 128, "vocab_size": 100, "max_seq_len": 512,
        ...     "early_fusion": True, "num_patches": 50, "patch_size": 100, "num_leads": 12})
    ...
    Example 2 -- early_fusion==False:
        >>> import torch
        >>> from omegaconf import OmegaConf
        >>> cfg = OmegaConf.create({"token_dim": 128, "vocab_size": 100,
        ...     "max_seq_len": 512, "early_fusion": False, "num_patches": 50,
        ...     "patch_size": 100, "num_leads": 12})
        >>> encoder = ECGTripletEncoder(cfg)
        >>> # Create a dummy batch
        >>> #   - Batch size: 2
        >>> #   - Sequence length: 10
    ...
    def encode(self, batch):
        ...
        time_emb = self.embed_func(self.date_embedder, batch["time_delta_days"]) * ~(
            batch["static_mask"]
        ).unsqueeze(dim=1)
        ...
        val_emb = self.embed_func(self.numeric_value_embedder, batch["numeric_value"]) * batch[
            "numeric_value_mask"
        ].unsqueeze(dim=1)
        ...
        if not self.cfg.early_fusion:
            ...
            modality_batch_idx = torch.tensor(
                batch["modality_batch_idx"], dtype=torch.long, device=ecg_emb.device
            )
            ...
            counts.index_add_(
                0, modality_batch_idx, torch.ones(len(modality_batch_idx), 1, device=ecg_emb.device)
            )
