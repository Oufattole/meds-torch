...
class EveryQueryDataset(PytorchDataset):
    def __init__(self, cfg, split):
        ...
        assert self.config.min_query_duration < self.config.max_query_duration
        assert self.config.query_sampling_strategy in ('within_record','random','fixed')
        if self.config.query_sampling_strategy == 'fixed': 
            assert self.config.query_fixed_duration is not None
            assert self.config.query_fixed_duration >= 0
            assert self.config.query_fixed_offset is not None 
            assert self.config.query_fixed_offset >= 0 
    ...
    def sample_query_duration(self, times): 
        ...
        match self.config.query_sampling_strategy: 
            ...
            case 'random': 
                query_duration = np.random.randint(self.config.min_query_duration, self.config.max_query_duration)
            case 'fixed': 
        ...
        if self.config.normalize_query: 
            normalized_query_duration = (query_duration - self.config.min_query_duration) / (self.config.max_query_duration - self.config.min_query_duration)
        else: 
    ...
    def sample_query_offset(self, times, censored, input_end_idx, query_duration):
        ...
        if self.config.normalize_query: 
            normalized_query_offset = (query_offset - self.config.min_query_offset) / (self.config.max_query_offset - self.config.min_query_offset)
        else: 
            ...
            for j in range(len(query_window_codes[i])): 
                if query_window_codes[i][j] == query['idx']:
                    if query['has_value']: 
                        ...
                        if x is None: continue # todo: is None used for outlier removal?
    ...
    def _seeded_getitem(self, idx: int) -> dict[str, list[float]]:
        ...
        ''' (issue)
        given very large self.config.max_seq_len
        given self.config.subsequence_sampling_strategy=FROM_START
        can we assume the full record is returned?
        '''
        ...
        query_code = {
            ...
            'range_min':0.4,
            'range_max':0.7,
            # can also inlcude code name and code type 
        }
        query = query_code | {
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Test case 3: Query extends beyond last time point (issue: should this raise error if censored is false?)
        (
            7,
            5,
            2,
...
def test_get_query_indices(
    ...
    if expected_exception is not None:
        with pytest.raises(expected_exception):
            query_start_idx, query_end_idx = dataset.get_query_indices(times, query_offset, query_duration, input_end_idx)
    else:
        query_start_idx, query_end_idx = dataset.get_query_indices(times, query_offset, query_duration, input_end_idx)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {'query_sampling_strategy': 'within_record', 'min_query_offset': 2, 'max_query_offset': 5, 'normalize_query': False},
            6,
            3,
            (False, 0, 0)
        ),
        ...
        (
            {'query_sampling_strategy': 'within_record', 'min_query_offset': 2, 'max_query_offset': 5, 'normalize_query': True},
            3,
            2,
        ...
        (
            {'query_sampling_strategy': 'random', 'min_query_offset': 2, 'max_query_offset': 5, 'normalize_query': False},
            3,
            2,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {'query_sampling_strategy': 'within_record', 'min_query_duration': 2, 'max_query_duration': 5, 'normalize_query': False},
            (False, 2, 2)

        ),
        # Test case 4: Within_record strategy, max_valid_query_duration (8) less than min_query_duration
        (
            {'query_sampling_strategy': 'within_record', 'min_query_duration': 15, 'max_query_duration': 20, 'normalize_query': False},
            (False, 8, 8)

        ),
        # # Test case 5: Within_record strategy with normalization
        (
            {'query_sampling_strategy': 'within_record', 'min_query_duration': 2, 'max_query_duration': 5, 'normalize_query': True},
            (False, 2, (2-2)/(5-2))
        ),
        # # Test case 6: Random strategy with random seed
        (
            {'query_sampling_strategy': 'random', 'min_query_duration': 2, 'max_query_duration': 5, 'normalize_query': False},
            (False, 2, 2) 
        ),
    ],
)
...
def test_pytorch_dataset(meds_dir: Path, collate_type):
    ...
    else: raise NotImplementedError(f"{collate_type} not implemented")