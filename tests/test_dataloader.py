...
TRAIN_0_SCHEMA = {
    ...
    "code": [[10, 8], [10, 7]],
    "numerical_value": [[1.577007, None], [0.068028, None]],
    "start_time": [
        "1980-12-28 00:00:00",
        "1978-06-20 00:00:00",
    ],
...
TUNING_0_SCHEMA = {
    ...
    "timestamp": [["2010-01-03 06:27:59", "2010-01-03 08:22:13", "1988-12-19 00:00:00"]],
}
    ...
    schema = schema.with_columns(
        [
            ...
            pl.col("timestamp").list.eval(
                pl.element().str.strptime(pl.Datetime, format="%Y-%m-%d %H:%M:%S"), parallel=True
            ),
        ]
    )
    return schema
...
TRAIN_0_NRT_DATA = {
    ...
    "dim1/time_delta_days": [
        np.array(
            [np.nan, 1.0726737e04, 4.8263888e-03, 2.5543982e-02, 2.2025462e-02, 2.0844907e-02],
            dtype=np.float32,
        ),
        np.array(
            [
    ...
    "dim2/code": [
        ...
        np.array([12.0, 2.0, 11.0], dtype=np.float32),
        np.array([12.0, 11.0], dtype=np.float32),
        np.array([12.0, 11.0], dtype=np.float32),
...
TRAIN_1_NRT_DATA = {
    ...
    "dim1/time_delta_days": [
        np.array([np.nan, 1.17661045e04, 9.78703722e-02], dtype=np.float32),
        np.array([np.nan, 1.2367247e04, 4.6423610e-02], dtype=np.float32),
    ],
    "dim2/bounds": np.array([1, 4, 5, 6, 9, 10]),
    "dim2/code": [
        np.array([6.0], dtype=np.float32),
        np.array([11.0, 4.0, 12.0], dtype=np.float32),
        np.array([5.0], dtype=np.float32),
        np.array([6.0], dtype=np.float32),
        np.array([12.0, 11.0, 3.0], dtype=np.float32),
        np.array([5.0], dtype=np.float32),
    ],
...
def test_clmbr(tmp_path):
    ...
    config = {"embedder.token_dim": 4}
    with initialize(version_base=None, config_path="../src/MEDS_torch/configs"):  # path to config.yaml
        overrides = [f"{k}={v}" for k, v in config.items()]
        cfg = compose(config_name="base_model_config", overrides=overrides)  # config.yaml
