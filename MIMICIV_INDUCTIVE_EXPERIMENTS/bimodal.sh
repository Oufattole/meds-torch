...
export OUTPUT_DIR="/ssd-shared/ryan_results/ehr_clip/results" 
export TEXT_ROOT="/ssd-shared/physionet.org/files/mimic-iv-note/2.2/note/discharge.csv.gz"
export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7
export TORCH_DISTRIBUTED_DEBUG="DETAIL"

export METHOD=bicl
MAX_POLARS_THREADS=4 meds-torch-train \
    data.dataloader.num_workers=16 \
    experiment=triplet_bimodal_mtr \
    paths.data_dir=${MIMICIV_TRIPLET_TENSOR_DIR} \
    paths.meds_cohort_dir=${MIMICIV_MEDS_COHORT_DIR} \
    paths.output_dir=${OUTPUT_DIR} \
    model=$METHOD data=bimodal_pytorch_dataset \
    data.task_root_dir=${TASKS_DIR} data.text_root=$TEXT_ROOT\
...
#     hparams_search.ray.resources_per_trial.gpu=2 data.dataloader.num_workers=16 \
