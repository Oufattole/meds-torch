...
main_split = femr.splits.generate_hash_split(index.get_patient_ids(), 97, frac_test=0)
...
# Note, we can use a hierarchical tokenizer for MOTOR
# Our data does not have an ontology, and this still seems to work for it
with open("tests/processed/ontology.pkl", "rb") as f:
...
print(np.unique([each["type"] for each in tokenizer.dictionary["vocab"]]))
# Save the tokenizer to the same directory as the model
tokenizer.save_pretrained(os.path.join(tmp_path, "motor_model"))


# Second, we need to prefit the MOTOR model.
# This is necessary because piecewise exponential models are unstable without an initial fit

# set the number of tasks to be the number of codes... I think???
motor_task = femr.models.tasks.MOTORTask.fit_pretraining_task_info(
    main_dataset["train"], tokenizer, num_tasks=8, num_bins=4, final_layer_size=32, num_proc=1
)
...
train_batches = processor.convert_dataset(
    train_dataset, tokens_per_batch=1, num_proc=1, min_patients_per_batch=1
)
...
model.save_pretrained(os.path.join(tmp_path, "motor_model"))
shutil.rmtree(tmp_path)
